		...
		for (int n = 0; (unsigned)n < nodos.size(); n++) {			//por cada nivel
			for (int e = 0; (unsigned)e < nodos[n].size(); e++) {	//para cada elemento
				...
				for (int i = 0; i < 15; i++){
					...
					else if (nodos[n][e].text == "1") {
						variable = 'B';
					}
					else if (nodos[n][e].text == "2") {
						variable = 'C';
					}
					else if (nodos[n][e].text == "3") {
						variable = 'D';
					}
					else if (nodos[n][e].text == "4") {
						variable = 'E';
					}
					else if (nodos[n][e].text == "5") {
						variable = 'F';
					}
					else if (nodos[n][e].text == "6") {
						variable = 'G';
					}
					else if (nodos[n][e].text == "7") {
						variable = 'H';
					}
					else if (nodos[n][e].text == "8") {
						variable = 'I';
					}
					else if (nodos[n][e].text == "9") {
						variable = 'J';
					}
					else if (nodos[n][e].text == "10") {
						variable = 'K';
					}
					else if (nodos[n][e].text == "11") {
						variable = 'L';
					}
					else if (nodos[n][e].text == "12") {
						variable = 'M';
					}
					else if (nodos[n][e].text == "13") {
						variable = 'N';
					}
					else if (nodos[n][e].text == "14") {
						variable = 'O';
					}
					else if (nodos[n][e].text == "15") {
						variable = 'P';
					}
					
				}
				/*nodos[n][e].text.c_str()*/
				g->FillEllipse(brush0, nodos[n][e].x + x, nodos[n][e].y + y, C::NODO::DIAMETRO, C::NODO::DIAMETRO);
				g->DrawString(gcnew String(variable,1), font, brush1, nodos[n][e].x + x, nodos[n][e].y + y);
			}
		}
